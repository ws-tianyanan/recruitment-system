...
<div class="rightinfo">
  <div class="tools">
    <ul class="toolbar">
    <!-- /recruitment-system/WebContent/system_administrator/jsp/companyAdd.jsp -->
      <li class="click"><span><img src="${pageContext.servletContext.contextPath }/system_administrator/images/t01.png" /></span>
      <a href="${pageContext.servletContext.contextPath }/system_administrator/jsp/companyAdd.jsp">添加</a></li>
    </ul>
      ...
      <tr>
        <td style="height: 50px"><input name="" type="checkbox" value="" /></td>
         ...
         <td style="height: 50px">
          <c:if test="${company.companyState == 1 }">招聘中</c:if>
          <c:if test="${company.companyState == 2 }">已暂停</c:if>
          <c:if test="${company.companyState == 3 }">已结束</c:if>
         </td>
         <td style="height: 50px">${company.companySort }</td>
         <td style="height: 50px">${company.companyViewnum }</td>
         <td style="height: 50px">
          <c:if test="${company.companyHrId == 0 }"></c:if>
          <c:if test="${company.companyHrId != 0 }">${company.companyState }</c:if>
         </td>
         <td style="height: 50px">${company.companyTel }</td>
         <td style="height: 50px">
         	<a href="${pageContext.servletContext.contextPath }/ManagerCompanyServlet?type=updateByManager&companyId=${company.companyId }" class="tablelink">修  改</a> &nbsp;&nbsp;
         	<a href="${pageContext.servletContext.contextPath }/ManagerCompanyServlet?type=delectByManager&companyId=${company.companyId }" class="tablelink">删  除</a>
         </td>
       </tr>
      </c:forEach>
    </tbody>
